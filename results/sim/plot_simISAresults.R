...
for(Run in 1:10){
  for(r0 in 1:10){
    ...
    tab1 = rbind(tab1, tmp[1:12,])#tmp[1:80,])#
    tab2 = rbind(tab2, tmp[13:27,])#tmp[81:180,])#
  }
}
...
Alg = levels(tab2$Alg)[c(1,3,2)]
Algs = c('JBD SOS','RE+MISA','RE+MISA+GP')
...
p <- p + scale_shape_manual(values=c(2,1,0))
#p <- p + scale_size_manual(values=c(2,2,3))
p <- p + scale_color_manual(values=c('#e6ab02','#e7298a','#7570b3'))
...
print(p)
# Save pdf with Device: 6.70 (width) x 11.50 (height)
