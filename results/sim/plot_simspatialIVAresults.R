    ...
    try(
      {
        ...
        tab1 = rbind(tab1, tmp[1:16,])#
        tab2 = rbind(tab2, tmp[17:36,])#
      }
    )
  }
}
tab2 <- tab2[!is.na(tab2$SNRdB),] # Removes repeats that did not run
tab2[which(tab2$SNRdB == 0.004345118),]$SNRdB <- 0.0043451177401773
tab2[which(tab2$SNRdB == 0.457574906),]$SNRdB <- 0.457574905606751
tab2[which(tab2$SNRdB == 3.010299957),]$SNRdB <- 3.01029995663981
SNR = unique(tab2$SNRdB)
SNRs = makemynames(SNR)
Acond = unique(tab1$Acond)
Aconds = makemynames(Acond)
Alg = levels(tab2$Alg)[c(1,3,2,4)]
Algs = Alg
...
p <- p + scale_shape_manual(values=c(2,4,0,1))
#p <- p + scale_size_manual(values=c(2,2,3))
p <- p + scale_color_manual(values=c('#e6ab02','#1b9e77','#7570b3','#e7298a'))
