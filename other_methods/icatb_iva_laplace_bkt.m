...
for iter = 1:Params.maxIter
    ...
    for k=1:K
        [~, D] = eig(W(:,:,k)*W(:,:,k)');
        cost(iter)=cost(iter)+sum(log(abs(sqrt(diag(D)))));%log(abs(det(W(:,:,k))));
    end
    ...
    if iter>1
        alpha = Params.alpha0;
        while cost(iter) > (cost(iter-1) - c_*(alpha*sum(dW(:).^2)))
            if Params.verbose
                fprintf(1, repmat('\b',1,ct)); %delete line before
                ct = fprintf('Backtracking. Cost: %f, Alpha: %.30f', cost(iter), alpha);
            end
        ...
        normg = sum(dW(:).^2);
        Params.alpha0 = alpha*normg;
        %(alpha^2)*normg/(2*(cost(iter) - cost(iter-1) + alpha*normg));
        %-2*(cost(iter) - cost(iter-1))/sum(dW(:).^2);
        sqrtYtYInv=sqrtYtY.^(-1);
        dW=zeros(size(W));%dW=W*0;
        for k=1:K
        ...
        normg = sum(dW(:).^2);
        Params.alpha0 = Params.alpha0/normg;
        if mod(iter,100) == 0, Params.alpha0 = 1; end
    else
    ...
    if Params.verbose
        if supplyA
            fprintf(1, repmat('\b',1,ct)); %delete line before
            ct = fprintf('Step %d: W change: %f, Cost: %f, Avg ISI: %f, Joint ISI: %f',  ...
                iter, termCriterion,cost(iter),amari_avg_isi,amari_joint_isi);
        else
            fprintf(1, repmat('\b',1,ct)); %delete line before
            ct = fprintf('Step %d: W change: %f, Cost: %f, Alpha: %.30f, dWMax: %.15f', iter, termCriterion,cost(iter),alpha, max(dW(:)));
        end
    end % options.verbose
...
if iter==1 && Params.verbose
    if supplyA
        ...
        fprintf(1, repmat('\b',1,ct)); %delete line before
        fprintf('Step %d: W change: %f, Cost: %f', iter, termCriterion,cost(iter));
    end
end % options.verbose
