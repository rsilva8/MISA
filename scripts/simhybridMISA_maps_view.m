...
COMPS = 1:6;
sfile = fullfile(scripath,'MCIv4','ch2better_aligned2EPI.nii');
bname_GT = 'simhybridMISA_maps_m2_GT.nii';
fname_GT = fullfile(outpath,bname_GT);
bname_RM = 'simhybridMISA_maps_m2_RE_WT_MISA.nii';
...
export_fig(hFc,fullfile(outpath,'simhybridMISA_corr.pdf'),'-pdf','-opengl')
...
addpath(genpath('~/GSU/software/GIFT/GroupICATv4.0b/icatb'))
...
load('./scripts/MCIv4/CM_coldhot_256.mat', 'CM')
...
%%
hF = figure('position',[1 1 1440 804],'Renderer', 'opengl');
...
%Annotations
ax_a = axes;
title('Ground-Truth (GT) vs RE_WT+MISA Estimate')
hold(ax_a,'on')
axis(ax_a,'off')
axis image xy
total = size(allfmaps);
...
export_fig(hF,fullfile(outpath,'simhybridMISA_maps.pdf'),'-pdf','-opengl')
